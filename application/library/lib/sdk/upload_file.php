...
use app\library\lib\sdk\ResizeImage;
...
class upload_file{
    ...
    public function upfile($field,$video_type=''){
        ...
        if($this->upload_file['size'] > $this->max_size*1024*5){
        ...
        if ($ifresize){
            for ($i=0;$i<count($thumb_width);$i++){
				...
				$image_info = @getimagesize($this->upload_file['tmp_name']);
...
//				$resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
    ...
    private function fileInputError(){
        switch($this->upload_file['error']) {
            ...
            case 4:
                //没有文件被上传
                $this->setError('文件没有被上传');
                return false;
                break;

            case 6:
                //找不到临时文件夹
                $this->setError('文件上传失败,code:6');
                return false;
                break;

            case 7:
                //文件写入失败
                $this->setError('文件上传失败,code:7');
                return false;
                break;
